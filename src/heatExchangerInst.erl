...
-spec create(pid(), pid(), pid(), number()) -> {'ok', pid()}.
...
-spec init(pid(), pid(), pid(), number()) -> no_return().
...
-spec temp_influence(pid()) -> {'ok',_}|{'error', 'timed_out', pid(), _, reference()}.
temp_influence(HeatExchangerInst_Pid) -> 
	msg:get(HeatExchangerInst_Pid, get_temp_influence).

-spec loop(pid(), map(), pid(), pid(), number()) -> no_return().
loop(Host, State, HeatExchangerTyp_Pid, PipeInst_Pid, HE_link_spec) -> 
	receive
		{get_type, ReplyFn} -> 
