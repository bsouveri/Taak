...
-spec get_initial_state(pid(), pid(), any()) -> {'ok',_}|{'error', 'timed_out', pid(), _, reference()}.
...
-spec get_connections_list(pid, map()) -> {'ok',_}|{'error', 'timed_out', pid(), _, reference()}.
get_connections_list(ResTyp_Pid, State) -> 
	msg:get(ResTyp_Pid, connections_list, State).

-spec get_locations_list(pid(), map()) -> {'ok',_}|{'error', 'timed_out', pid(), _, reference()}.
